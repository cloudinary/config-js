...
> {
  const input: operations.GetMetadataFieldRequest = {
    externalId: externalId,
  };

  const parsed = safeParse(
    input,
    (value) => operations.GetMetadataFieldRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.MetadataField$inboundSchema),
